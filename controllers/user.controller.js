  ...
  try {
    ...
    if (blocked && !(blocked === "0" || blocked === "1")) {
      return res
        ...
        .json({
          message:
            "The 'blocked' parameter must be a boolean value ('0' for false and '1' for true)",
        });
    }
    ...
    if (isNaN(limit) || limit <= 5) {
      return res
        .status(400)
        .json({ message: "Limit must be a positive integer, greater than 5" });
    }

    if (isNaN(currentPage) || currentPage < 1) {
      return res
        .status(400)
        .json({ message: "Page must be 1 or a positive integer" });
    }
    ...
    const nextPage =
      currentPage < totalPages ? currentPage + 1 : currentPage === 1 ? 2 : null;
...
exports.findOne = async (req, res) => {
  try {
    ...
    const totalOwnedProperties = await db.property.count({
      where: { owner_id: userId },
    });

    // Count the total number of properties rented by the user
    const totalRentedProperties = await db.booking.count({
    ...
    for (const property of userProperties) {
      ...
      const { count: totalRentedProperties, rows: bookingsWithReviews } =
        await db.booking.findAndCountAll({
          where: {
      ...
      totalRatings += bookingsWithReviews.reduce(
        (acc, booking) => acc + booking.number_stars,
        0
      );
    }
    ...
    const averagePropertyRating =
      totalPropertyReviews > 0 ? totalRatings / totalPropertyReviews : 0;
...
exports.delete = async (req, res) => {
  try {
    ...
    if (
      ...
      loggedInUser.user_id !== parseInt(req.params.user_id)
    ) {
...
exports.updateCurrent = async (req, res) => {
  try {
    ...
    if (req.body.type) {
      return res.status(403).json({
        success: false,
        msg: `You are not authorized to change the user type.`,
      });
    }
...
exports.verifyEmail = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      msg:
        err.message || "Some error occurred while verifing the user's email.",
    });
  }
};
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    /// Read the email template from file
    const emailTemplate = fs.readFileSync(
      "./html/email_forgot_password.html",
      "utf8"
    );
    ...
    res.status(500).json({
      error:
        err.msg ||
        "Some error occurred while sending the reset password email.",
    });
  }
};
  ...
  try {
    ...
    properties = properties.map((property) => {
      ...
      plainProperty.links = [
        {
          rel: "self",
          href: `/properties/${plainProperty.property_id}`,
          method: "GET",
        },
        {
          rel: "delete",
    ...
    res.status(200).json({
      success: true,
      data: properties,
      links: [{ rel: "add-property", href: `/properties`, method: "POST" }],
    });
  } catch (err) {
    // If an error occurs, return a 500 response with an error message
    return res.status(500).json({
  ...
  try {
    ...
    bookings = bookings.map((booking) => {
      ...
      plainBooking.links = [
        {
          rel: "self",
          href: `/bookings/${plainBooking.booking_id}`,
          method: "GET",
        },
        {
          rel: "delete",
    ...
    res.status(200).json({
      success: true,
      data: bookings,
      links: [{ rel: "add-booking", href: `/bookings`, method: "POST" }],
    });
  } catch (err) {
    // If an error occurs, return a 500 response with an error message
    return res.status(500).json({
...
exports.addFavorite = async (req, res) => {
  try {
    ...
    const propertyAlreadyFavorited = user.favoriteProperty.some(
      (property) => property.property_id === req.body.property_id
    );
    ...
    // If an error occurs, return a 500 response with the error message
      ...
      return res.status(500).json({
        ...
        msg:
          err.message ||
          "Some error occurred while adding property to the favorites.",
      });
  }
};
...
exports.deleteFavorite = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      ...
      msg:
        err.message ||
        "Some error occurred while removing property from favorites.",
    });
  }
};
